...
def _screen(db, lot, actor, grade="B", confidence=80.0, flags=None):
    ...
    _audit(
        ...
        "Lot",
        lot.id,
        new_value={"predicted_grade": grade, "confidence_score": confidence},
    )
    return screening
...
def _make_offer(
    ...
    *,
    ...
    _audit(
        ...
        "Offer",
        offer.id,
        new_value={"lot_id": lot.id, "gross_price_per_kg": gross},
    )
    return offer
...
def _assign_shipment(
    ...
    *,
    ...
    _audit(
        ...
        new_value={"lot_id": lot.id, "vehicle_number": vehicle_number},
    )
    ...
    if (
        status in (ShipmentStatus.PICKED_UP, ShipmentStatus.IN_TRANSIT)
...
def _book_storage(db, lot, facility, actor, *, qty, days=10):
    ...
    _audit(
        ...
        "StorageBooking",
        lot.id,
        new_value={"facility_id": facility.id, "booked_quantity_kg": qty},
    )
    return booking


def _sensor_events(
    db, facility, booking, *, count=6, base_temp=27.0, base_humidity=67.0, alert_at=None
):
...
def _confirm_delivery(
    db, lot, po, payment, actor, *, delivered_grade="B", contamination=False
):
    ...
    _audit(
        ...
        "PurchaseOrder",
        po.id,
        new_value={"delivered_grade": delivered_grade, "outcome": outcome.outcome},
    )
    return outcome
...
def _pay(db, payment, actor, amount):
    ...
    _audit(
        ...
        "Payment",
        payment.id,
        new_value={"amount": amount, "status": payment.status.value},
    )
...
def _resolve_dispute(
    ...
    *,
    ...
    if adjustment and settlement:
        ...
        lines = compute_settlement_items(
            contributors=[
                ContributorShare(item.farmer_id, item.contributed_quantity_kg)
    ...
    _audit(
        ...
        "Dispute",
        dispute.id,
        new_value={"status": status.value, "financial_adjustment": adjustment},
    )
    ...
    try:
        ...
        db.add(
            FPOAgentProfile(
                user_id=fpo_user.id,
                fpo_id=fpo.id,
                designation="Collection Centre Agent",
            )
        )
        ...
        fpo1_farmer_defs = [
            ...
            (
                "prakash.sonawane@annadata.demo",
                "Prakash Sonawane",
                "9990000109",
                "Vinchur",
                6.0,
        ...
        fpo2_farmer_defs = [
            (
                "kalpana.salunkhe@annadata.demo",
                "Kalpana Salunkhe",
                "9990000114",
                "Yeola",
                3.8,
        ...
        buyer_defs = [
            ...
            (
                "buyer4@annadata.demo",
                "Konkan Fresh Exports",
                BuyerType.EXPORTER,
                "27ABCDE4321F1Z8",
                VerificationStatus.VERIFIED,
            ),
            (
                "buyer5@annadata.demo",
                "Deccan Agri Traders",
                BuyerType.WHOLESALER,
                "27ABCDE1111F1Z3",
            ...
            (
                "buyer6@annadata.demo",
                "Sahyadri Retail Chain",
                BuyerType.RETAILER,
                "27ABCDE2222F1Z7",
        ...
        db.add(assayer)
        assayer2_user = _user(
            db, "assayer2@annadata.demo", "Assayer B", UserRole.ASSAYER, "9990000302"
        )
        assayer2 = AssayerProfile(
            user_id=assayer2_user.id,
            certification_id="ASSAY-CERT-DEMO-02",
            affiliated_fpo_id=fpo2.id,
        )
        db.add(assayer2)
        db.flush()
        ...
        db.add(transporter)
        transporter2_user = _user(
            db,
            "transporter2@annadata.demo",
            "Lasalgaon Carriers (Dispatch)",
            UserRole.TRANSPORTER,
            "9990000402",
        )
        transporter2 = TransporterProfile(
            user_id=transporter2_user.id,
            company_name="Lasalgaon Carriers",
            contact_phone="9990000402",
        )
        db.add(transporter2)
        db.flush()
        ...
        print(
            "Reference data created (2 FPOs, 18 farmers, 6 buyers, 2 assayers, 2 transporters, 3 storage facilities)."
        )
        ...
        # =====================================================================
        flagship_contributors = [
            (f1["Kavita Shinde"], 420.0),
            (f1["Ganesh Patil"], 680.0),
            (f1["Rajendra Jadhav"], 900.0),
        ]
        lot = _new_lot(
            db,
            fpo=fpo,
            commodity=onion,
            contributors=flagship_contributors,
            village="Niphad",
        ...
        _screen(
            ...
            grade="B",
            confidence=82.0,
            flags=["minor visible defects", "surface damage"],
        )
        _assess(
            db,
            lot,
        ...
        offer_a = _make_offer(
            db,
            lot,
            buyers["Nashik Food Processing Unit"],
            buyer_users["Nashik Food Processing Unit"],
            gross=30.0,
            transport=2.0,
            loading=0.75,
            grading=0.5,
        ...
        offer_b = _make_offer(
            db,
            lot,
            buyers["Maharashtra Retail Procurement Network"],
            buyer_users["Maharashtra Retail Procurement Network"],
            gross=29.0,
            transport=0.75,
            loading=0.4,
            grading=0.25,
        ...
        db.add(
            DisputeEvidence(
                ...
                uploaded_by_user_id=buyer_users[
                    "Maharashtra Retail Procurement Network"
                ].id,
            )
        )
        ...
        db.commit()
        print(
            f"  {lot.lot_code}: {lot.status.value} (flagship — 2,000kg, resolved dispute)"
        )
        ...
        _new_lot(
            ...
            fpo=fpo,
            commodity=onion,
            contributors=[(f1["Sunita Pawar"], 380.0), (f1["Vitthal Gaikwad"], 720.0)],
            village="Lasalgaon",
            collection_point="Lasalgaon Sub-Centre",
            variety="Bhima Super",
            created_by=fpo_user,
            days_ago=1,
        )
        _new_lot(
            ...
            fpo=fpo2,
            commodity=onion,
            contributors=[(f2["Kalpana Salunkhe"], 500.0), (f2["Ramesh Aher"], 545.0)],
            village="Yeola",
            collection_point="Yeola Collection Centre",
            variety="Agrifound Dark Red",
            created_by=fpo2_user,
            days_ago=1,
        )
        ...
        lot4 = _new_lot(
            ...
            fpo=fpo,
            commodity=onion,
            contributors=[(f1["Anita Kadam"], 310.0), (f1["Shivaji Kale"], 560.0)],
            village="Pimpalgaon Baswant",
            collection_point="Pimpalgaon Baswant Collection Point",
            variety="Nashik Red N-53",
            created_by=fpo_user,
            days_ago=3,
        )
        ...
        lot5 = _new_lot(
            ...
            commodity=onion,
            contributors=[
                (f1["Meera Bhosale"], 470.0),
                (f1["Prakash Sonawane"], 810.0),
            ],
            village="Datyane",
            collection_point="Datyane Collection Point",
            variety="Lasalgaon Red (GI)",
            created_by=fpo_user,
            days_ago=4,
        )
        _collect(db, lot5, fpo_user)
        _screen(
            db, lot5, fpo_user, grade="B", confidence=77.5, flags=["size variation"]
        )
        ...
        lot6 = _new_lot(
            ...
            fpo=fpo,
            commodity=onion,
            contributors=[(f1["Lata Wagh"], 340.0), (f1["Dilip Chavan"], 610.0)],
            village="Bhendali",
            collection_point="Bhendali Collection Point",
            variety="Bhima Super",
            created_by=fpo_user,
            days_ago=5,
        )
        ...
        _assess(
            ...
            935.0,
            sample_kg=12.0,
            notes="Uniform bulb size, well cured.",
        )
        ...
        lot7 = _new_lot(
            ...
            fpo=fpo,
            commodity=onion,
            contributors=[(f1["Sarika Thorat"], 405.0), (f1["Bhausaheb Gite"], 690.0)],
            village="Lasalgaon",
            collection_point="Lasalgaon Sub-Centre",
            variety="Nashik Red N-53",
            created_by=fpo_user,
        ...
        _assess(
            ...
            1080.0,
            sample_kg=14.0,
            notes="Acceptable moisture level; minor size variation.",
        )
        ...
        _make_offer(
            db,
            lot7,
            buyers["Regional Onion Wholesaler"],
            buyer_users["Regional Onion Wholesaler"],
            gross=25.5,
            transport=1.0,
            loading=0.3,
            qty=1080.0,
        ...
        _make_offer(
            db,
            lot7,
            buyers["Konkan Fresh Exports"],
            buyer_users["Konkan Fresh Exports"],
            gross=29.5,
            transport=4.2,
            loading=0.8,
            grading=0.5,
            ...
            qty=1080.0,
            grade="B",
            offer_type=OfferType.PURCHASE_ORDER,
        )
        ...
        lot8 = _new_lot(
            ...
            fpo=fpo2,
            commodity=onion,
            contributors=[(f2["Pushpa Bhagat"], 360.0), (f2["Nivrutti Shelke"], 760.0)],
            village="Chandwad",
            collection_point="Yeola Collection Centre",
            variety="Agrifound Dark Red",
            created_by=fpo2_user,
        ...
        _assess(
            ...
            1105.0,
            sample_kg=13.0,
            notes="Excellent bulb uniformity and dry outer skin.",
        )
        ...
        lot9 = _new_lot(
            ...
            fpo=fpo2,
            commodity=onion,
            contributors=[(f2["Yamuna Kolhe"], 400.0)],
            village="Yeola",
            collection_point="Yeola Collection Centre",
            variety="Agrifound Dark Red",
            created_by=fpo2_user,
        ...
        lot10 = _new_lot(
            ...
            fpo=fpo,
            commodity=onion,
            contributors=[(f1["Kavita Shinde"], 300.0), (f1["Ganesh Patil"], 400.0)],
            village="Niphad",
            collection_point="Niphad Collection Centre",
            variety="Nashik Red N-53",
            created_by=fpo_user,
        ...
        _create_po(
            ...
            fpo,
            fpo_user,
            delivery_location="Nashik Food Processing Unit, MIDC Ambad, Nashik",
            quantity_kg=690.0,
        )
        ...
        lot11 = _new_lot(
            ...
            fpo=fpo,
            commodity=onion,
            contributors=[(f1["Rajendra Jadhav"], 500.0), (f1["Sunita Pawar"], 300.0)],
            village="Niphad",
            collection_point="Niphad Collection Centre",
            variety="Nashik Red N-53",
            created_by=fpo_user,
        ...
        po11, payment11 = _create_po(
            db,
            lot11,
            o11,
            fpo,
            fpo_user,
            delivery_location="Regional Onion Wholesaler Yard, Panchavati, Nashik",
            quantity_kg=780.0,
            payment_status=PaymentStatus.DISPATCHED,
        )
        _assign_shipment(
            db,
            lot11,
            ...
            transporter2,
            transporter2_user,
            vehicle_number="MH-15-CT-7812",
            vehicle_type="Covered tempo, 3 tonne",
            capacity_kg=3000,
            driver_contact="9990000498",
            pickup_point="Niphad Collection Centre",
            status=ShipmentStatus.IN_TRANSIT,
            hours_ago=2,
        )
        booking11 = _book_storage(
            db, lot11, storage_facility2, fpo_user, qty=780.0, days=5
        )
        _sensor_events(
            db,
            storage_facility2,
        ...
        lot12 = _new_lot(
            ...
            fpo=fpo,
            commodity=onion,
            contributors=[(f1["Vitthal Gaikwad"], 400.0), (f1["Anita Kadam"], 310.0)],
            village="Niphad",
            collection_point="Niphad Collection Centre",
            variety="Nashik Red N-53",
            created_by=fpo_user,
        ...
        _assign_shipment(
            ...
            transporter,
            transporter_user,
            vehicle_number="MH-15-BT-3390",
            vehicle_type="Open truck, 3 tonne",
            capacity_kg=3000,
            driver_contact="9990000499",
            pickup_point="Niphad Collection Centre",
            status=ShipmentStatus.DELIVERED,
        ...
        _initiate_payment(
            db, payment12, buyer_users["Nashik Food Processing Unit"], days_ago=2
        )
        _pay(
            db,
            payment12,
            buyer_users["Nashik Food Processing Unit"],
            round(payment12.amount_due * 0.45, 2),
        )
        ...
        lot13 = _new_lot(
            ...
            commodity=onion,
            contributors=[
                (f1["Shivaji Kale"], 560.0),
                (f1["Meera Bhosale"], 470.0),
                (f1["Prakash Sonawane"], 300.0),
            ],
            village="Khede",
            collection_point="Khede Collection Point",
            variety="Lasalgaon Red (GI)",
            created_by=fpo_user,
            days_ago=18,
        )
        ...
        _assign_shipment(
            ...
            transporter,
            transporter_user,
            vehicle_number="MH-15-BT-6620",
            vehicle_type="Open truck, 5 tonne",
            capacity_kg=5000,
            driver_contact="9990000499",
            pickup_point="Khede Collection Point",
            status=ShipmentStatus.DELIVERED,
            hours_ago=96,
        )
        ...
        _assess(
            ...
            1030.0,
            sample_kg=14.0,
            notes="Export-quality bulbs, minimal blemish.",
        )
        ...
        po14, payment14 = _create_po(
            db,
            lot14,
            o14,
            fpo2,
            fpo2_user,
            delivery_location="Konkan Fresh Exports Packhouse, JNPT Road, Navi Mumbai",
            quantity_kg=1030.0,
            po_status=POStatus.FULFILLED,
        )
        _assign_shipment(
            ...
            transporter2,
            transporter2_user,
            vehicle_number="MH-15-CT-9021",
            vehicle_type="Refrigerated-capable tempo, 5 tonne",
            capacity_kg=5000,
            driver_contact="9990000498",
            pickup_point="Yeola Collection Centre",
            status=ShipmentStatus.DELIVERED,
            hours_ago=72,
        )
        _confirm_delivery(db, lot14, po14, payment14, admin_user, delivered_grade="A")
        _initiate_payment(
            db, payment14, buyer_users["Konkan Fresh Exports"], days_ago=4
        )
        ...
        _settle(
            ...
            lot14,
            payment14,
            [(f2["Kalpana Salunkhe"], 500.0), (f2["Ramesh Aher"], 545.0)],
            fpo2_user,
        )
        ...
        _assign_shipment(
            ...
            transporter,
            transporter_user,
            vehicle_number="MH-15-BT-1150",
            vehicle_type="Open truck, 3 tonne",
            capacity_kg=3000,
            driver_contact="9990000499",
            pickup_point="Bhendali Collection Point",
            status=ShipmentStatus.DELIVERED,
            hours_ago=48,
        )
        _confirm_delivery(db, lot15, po15, payment15, admin_user, delivered_grade="B")
        _initiate_payment(
            db, payment15, buyer_users["Nashik Food Processing Unit"], days_ago=2
        )
        _pay(
            db,
            payment15,
            buyer_users["Nashik Food Processing Unit"],
            payment15.amount_due,
        )
        _settle(
            db,
            lot15,
            payment15,
            [(f1["Lata Wagh"], 340.0), (f1["Dilip Chavan"], 610.0)],
            fpo_user,
        )
        dispute15 = _raise_dispute(
            db,
            lot15,
            po15,
            buyer_users["Nashik Food Processing Unit"],
            "Roughly one-fifth of the delivered bags were under-weight compared to the manifest; requesting re-weighment and a price adjustment.",
            status=DisputeStatus.UNDER_REVIEW,
        )
        db.add(
            DisputeEvidence(
                dispute_id=dispute15.id,
                evidence_type="NOTE",
                description="Weighbridge slip at buyer warehouse shows 6% shortfall versus the purchase order quantity.",
                uploaded_by_user_id=buyer_users["Nashik Food Processing Unit"].id,
            )
        )
        ...
        lot16 = _new_lot(
            ...
            commodity=onion,
            contributors=[
                (f2["Pushpa Bhagat"], 360.0),
                (f2["Nivrutti Shelke"], 400.0),
                (f2["Yamuna Kolhe"], 200.0),
            ],
            village="Yeola",
            collection_point="Yeola Collection Centre",
            variety="Agrifound Dark Red",
        ...
        _screen(
            ...
            grade="C",
            confidence=68.0,
            flags=["surface damage", "moisture spotting"],
        )
        _assess(
            db,
            lot16,
            ...
            sample_kg=12.0,
            defects=[
                "visible sprouting on ~15% of sample",
                "surface moisture spotting",
            ],
            notes="Below-target grade due to early sprouting; still usable for wholesale at a step-down price.",
        )
        ...
        _assign_shipment(
            ...
            transporter2,
            transporter2_user,
            vehicle_number="MH-15-CT-4477",
            vehicle_type="Open tempo, 3 tonne",
            capacity_kg=3000,
            driver_contact="9990000498",
            pickup_point="Yeola Collection Centre",
            status=ShipmentStatus.DELIVERED,
        ...
        _initiate_payment(
            db, payment16, buyer_users["Regional Onion Wholesaler"], days_ago=9
        )
        _pay(
            db,
            payment16,
            buyer_users["Regional Onion Wholesaler"],
            payment16.amount_due,
        )
        _settle(
            db,
            lot16,
        ...
        _audit(
            db, fpo2_user, "LOT_CLOSED", "Lot", lot16.id, new_value={"status": "CLOSED"}
        )

        db.commit()
        print(
            "  15 additional lots seeded across every workflow status (DRAFT through CLOSED)."
        )
        ...
        print(
            "  buyer        buyer6@annadata.demo (retailer, UNVERIFIED — cannot submit offers)"
        )
